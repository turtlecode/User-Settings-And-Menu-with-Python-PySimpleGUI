...
def main_window():
    ...
    layout = [[sg.MenubarCustom(menu_def, tearoff=False)],
              [sg.T("Input File:", s=15, justification="r"), sg.I(key="-IN-"),
               sg.FileBrowse(file_types=(("Excel Files", "*.xls*"),))],

              [sg.T("Output Folder:", s=15, justification="r"), sg.I(key="-OUT-"),
               sg.FolderBrowse()],

              [sg.Exit(s=16, button_color="tomato"),sg.B("Settings", s=16), 
              sg.B("Display Excel File", s=16), sg.B("Convert To CSV", s=16)],]
